...
output_folder = '/content/drive/MyDrive/CADRADS2/0_2/ex4'
model_path = '/content/drive/MyDrive/CADRADS2/all_class/ex6'
...
for data in train_test_split(img_data, cad_data, paz_data, ramo_data):
    ...
    model = model3(input_size = input_size)
    ...
    #metrics
    NPV0 = (np.sum(cnf_matrix[1][1:]) + np.sum(cnf_matrix[2][1:]))/ (np.sum(cnf_matrix[1][1:]) + np.sum(cnf_matrix[2][1:])+ np.sum(cnf_matrix[0][1:]))
    NPV1 = (cnf_matrix[0][0]+cnf_matrix[0][-1]+cnf_matrix[-1][0]+cnf_matrix[-1][-1]) / (cnf_matrix[0][0]+cnf_matrix[0][-1]+cnf_matrix[-1][0]+cnf_matrix[-1][-1]+cnf_matrix[1][0]+cnf_matrix[1][-1])
    NPV2 = (np.sum(cnf_matrix[0][0:2])+np.sum(cnf_matrix[1][0:2])) /(np.sum(cnf_matrix[0][0:2])+np.sum(cnf_matrix[1][0:2])+np.sum(cnf_matrix[2][0:2]))
    ...
    print_txt(out_fold, ['\n\n %s \n\n' % metrics.classification_report(test_cad, y, target_names=['Class 0', 'Class 1', 'Class 2'])])
    ...
    with open(out_file, "a") as text_file:
        ...
        for ii in range(len(prediction)):
            text_file.write(
                '%d                %.3f  %.3f  %.3f                 %d                  %d                %d\n' % (test_cad[ii], prediction[ii][0],prediction[ii][1],prediction[ii][2], y[ii], test_paz[ii], test_ramo[ii]))
    ...
    df1 = pd.DataFrame({'real_class': test_cad, 'pred_class': y, 'pred_prob0': pred0, 'pred_prob1': pred1, 'pred_prob2': pred2, 'paz': test_paz, 'ramo': test_ramo})
...
infer_macroPPV = round(mean_macroPPV - (round(stats.t.ppf(1-0.025, len(macroPPV)-1),3) * std_macroPPV / np.sqrt(len(macroPPV))),4)
upper_macroPPV = round(mean_macroPPV + (round(stats.t.ppf(1-0.025, len(macroPPV)-1),3) * std_macroPPV / np.sqrt(len(macroPPV))),4)

infer_weightedPPV = round(mean_weightedPPV - (round(stats.t.ppf(1-0.025, len(weightedPPV)-1),3) * std_weightedPPV / np.sqrt(len(weightedPPV))),4)
upper_weightedPPV = round(mean_weightedPPV + (round(stats.t.ppf(1-0.025, len(weightedPPV)-1),3) * std_weightedPPV / np.sqrt(len(weightedPPV))),4)

infer_macroNPV = round(mean_macroNPV - (round(stats.t.ppf(1-0.025, len(macroNPV)-1),3) * std_macroNPV / np.sqrt(len(macroNPV))),4)
upper_macroNPV = round(mean_macroNPV + (round(stats.t.ppf(1-0.025, len(macroNPV)-1),3) * std_macroNPV / np.sqrt(len(macroNPV))),4)

infer_weightedNPV = round(mean_weightedNPV - (round(stats.t.ppf(1-0.025, len(weightedNPV)-1),3) * std_weightedNPV / np.sqrt(len(weightedNPV))),4)
upper_weightedNPV = round(mean_weightedNPV + (round(stats.t.ppf(1-0.025, len(weightedNPV)-1),3) * std_weightedNPV / np.sqrt(len(weightedNPV))),4)

infer_macroF1 = round(mean_macroF1 - (round(stats.t.ppf(1-0.025, len(macroF1)-1),3) * std_macroF1 / np.sqrt(len(macroF1))),4)
upper_macroF1 = round(mean_macroF1 + (round(stats.t.ppf(1-0.025, len(macroF1)-1),3) * std_macroF1 / np.sqrt(len(macroF1))),4)

infer_weightedF1 = round(mean_weightedF1 - (round(stats.t.ppf(1-0.025, len(weightedF1)-1),3) * std_weightedF1 / np.sqrt(len(weightedF1))),4)
upper_weightedF1 = round(mean_weightedF1 + (round(stats.t.ppf(1-0.025, len(weightedF1)-1),3) * std_weightedF1 / np.sqrt(len(weightedF1))),4)

infer_macroRec = round(mean_macroRec - (round(stats.t.ppf(1-0.025, len(macroRec)-1),3) * std_macroRec / np.sqrt(len(macroRec))),4)
upper_macroRec = round(mean_macroRec + (round(stats.t.ppf(1-0.025, len(macroRec)-1),3) * std_macroRec / np.sqrt(len(macroRec))),4)

infer_weightedRec = round(mean_weightedRec - (round(stats.t.ppf(1-0.025, len(weightedRec)-1),3) * std_weightedRec / np.sqrt(len(weightedRec))),4)
upper_weightedRec = round(mean_weightedRec + (round(stats.t.ppf(1-0.025, len(weightedRec)-1),3) * std_weightedRec / np.sqrt(len(weightedRec))),4)
